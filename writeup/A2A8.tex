...
but near $t\approx T$ the traces reveal that Forward Euler (orange, dashed)
...
\begin{figure*}[!t]
  ...
  \caption{Harmonic oscillator trajectories and phase space at
    ...
    \textbf{(a)} Position $x(t)$ and \textbf{(b)} velocity $v(t)$: Euler
    (orange) exhibits visible phase lag near $t\in[9,10]$;
    ...
    Central zoom panel in \textbf{(c)} confirms the endpoint separation.}
  \label{fig:ho_traj}
\end{figure*}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{results1_figure2.png}
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.96\textwidth]{results1_figure3.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{results2_figure8.png}
...
Results~3 is a separate timing benchmark, not a repeat of the brief-mandated 100-step argon validation run from Results~2.
All scaling runs use Velocity-Verlet, fixed seed 42,
$\Delta t=\SI{e-14}{s}$, and no file output.
Strong scaling uses 1000 timed steps at fixed $N=2048$, whereas size scaling uses 1000 timed steps at fixed $P=16$; each configuration is repeated 20 times and the median paired wall/communication sample is retained to reduce measurement noise and fixed-overhead contamination.
All times reported in this section are raw measured seconds from those benchmark windows, not values normalised to 100 steps.
...
\begin{table}[ht]
  \centering
  \caption{Strong scaling results ($N=2048$, 1000 timed steps; medians of 20 no-output repetitions).
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.96\textwidth]{results3_figure10abc.png}
  \caption{Strong scaling ($N=2048$ fixed, Velocity-Verlet, 1000 timed steps;
    medians of 20 no-output repetitions, dual-socket Intel Xeon Silver 4314 node).
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.96\textwidth]{results3_figure9ab.png}
  \caption{Problem-size scaling ($P=16$ fixed, Velocity-Verlet, 1000 timed steps;
