...
Strong scaling uses 500 timed steps at fixed $N=2048$, whereas size scaling uses 2000 timed steps at fixed $P=16$; each configuration is repeated 20 times and the median paired wall/communication sample is retained to reduce measurement noise and fixed-overhead contamination.
...
\begin{table}[ht]
  \centering
  \caption{Strong scaling results ($N=2048$, 500 timed steps; medians of 20 no-output repetitions).
...
\begin{figure*}[!t]
  ...
  \caption{Problem-size scaling ($P=16$ fixed, Velocity-Verlet, 2000 timed steps;
